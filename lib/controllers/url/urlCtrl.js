...
function getUrlCode(req, res) {
  ...
  res.render('index', { host: local, title: 'Url Shortener' });
}
...
function loadMultipleUrl(urlArray, res, local) {
  const urlArrayLength = urlArray.length;
  let urlCount = 0;
  var connection;
  var ws = fs.createWriteStream(`${__dirname}/uploads/lastestshortenurls.csv`, { flags: 'w', defaultEncoding: 'utf8' })
    ...
    urlArray.forEach((urlObj) => {
      ...
      .then((bResult) => {
        if (bResult[0] != null) {
          ...
          var newUrl = { original_url: urlObj, short_url: shortCode };
          const results = connection.query('INSERT INTO URLS SET ?', newUrl).then((insertResult) => {
        ...
        if (urlArrayLength === urlCount) {
          res.render('upload', { uploaded: true });
        }
      }).catch((e) => {
        urlCount += 1;
...
function handleUrlUpload(req, res, next) {
  ...
  form.on('file', function (name, file){
    ...
    .on('close', () => {
      loadMultipleUrl(loadedUrls,res, local);
    });
   });
}
